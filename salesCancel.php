...
// 楽天へRMS APIを使って送信
list($request, $httpStatusCode, $response) = salesCancel($requestId, $uiCancelRequestModels);

//////////////// 関数群 ////////////////////

/***
* salesCancel APIのリクエストを行う。売上請求済の受注番号を元に売上請求の取消を行う
* SOAP Clientを使ってパラメータをセットしPOSTしている
* @param キャンセルしたい受注の決済のクラスオブジェクト
...
<SOAP-ENV:Envelope xmlns:SOAP-ENV="http://schemas.xmlsoap.org/soap/envelope/" xmlns:ns1="java:jp.co.rakuten.rms.mall.rccsapi.model.entity" xmlns:ns2="http://orderapi.rms.rakuten.co.jp/rccsapi-services/RCCSAPI" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
  <SOAP-ENV:Body>
    <ns2:salesCancel>
      ...
      <ns2:intVal>167311646</ns2:intVal>
...
*/
function salesCancel($requestId, $uiCancelRequestModels) {
  ...
  try{
    //SOAP通信実行
    $result = $client->salesCancel($params);
...
<html>
  <head>
    <title>salesCancel | PaymentAPI</title>
    <meta charset="UTF-8">
    <style>
      pre,code {
        width:100%;
