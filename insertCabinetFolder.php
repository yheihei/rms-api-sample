 ...
 * */
$folder = new Folder();
$folder->folderName = 'testrrrz_' . randomStr(3) . '_' . date_format(new DateTime('now', new DateTimeZone('Asia/Tokyo')), 'YmdHis');

// 楽天へRMS APIを使って登録
list($reqXml, $httpStatusCode, $response) = insertFolder($folder);
...
/*
* cabinet.folder.insert APIのリクエストを行う
* xmlを作って curlでpostしてる
* @param 挿入したいフォルダ情報のクラスオブジェクト
...
function insertFolder($folder) {
  ...
  $url = RMS_API_CABINET_FOLDERS_INSERT;
  $ch = curl_init($url);
  
  $reqXml = _createRequestXml($folder);
...
*/
function _createRequestXml($folder) {

  // リクエストXMLのガワを作る
  $rootXml = new SimpleXMLElement('<?xml version="1.0" encoding="UTF-8"?><request/>');
  $folderInsertRequestXml = $rootXml->addChild('folderInsertRequest');
  $folderXml = $folderInsertRequestXml->addChild('folder');
  
  // 受け取った商品情報オブジェクトをarrayに変換
  $array = _convertClassObjectToArray($folder);
  
  _arrayToXml($array, $folderXml);  // リクエストのXMLをarray情報から作成する
...
<html>
  <head>
    <title>cabinet.folder.insert | CabinetAPI</title>
    <meta charset="UTF-8">
    <style>
      pre,code {
        width:100%;
