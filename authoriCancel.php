...
/***
 * オーソリ取り消し処理 authoriCancel [非同期]
 * $requestIdはシステム内に保存しておくこと。
 * 非同期のため、処理が正しく行われたかどうかはgetRCCSResult APIに$requestIdを入れて確認する必要がある
 * */
$uiCancelRequestModel = new UiCancelRequestModel();
$uiCancelRequestModel->orderNumber = $orderNumber;
$uiCancelRequestModels[] = $uiCancelRequestModel;

// 楽天へRMS APIを使って送信
list($request, $httpStatusCode, $response) = authoriCancel($requestId, $uiCancelRequestModels);

//////////////// 関数群 ////////////////////

/*
* オーソリのキャンセルAPIのリクエストを行う
...
<SOAP-ENV:Envelope xmlns:SOAP-ENV="http://schemas.xmlsoap.org/soap/envelope/" xmlns:ns1="java:jp.co.rakuten.rms.mall.rccsapi.model.entity" xmlns:ns2="http://orderapi.rms.rakuten.co.jp/rccsapi-services/RCCSAPI" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
  <SOAP-ENV:Body>
    <ns2:authoriCancel>
      ...
      <ns2:intVal>167147102</ns2:intVal>
      <ns2:uiCancelRequestModels>
        <ns1:UiCancelRequestModel>
          <ns1:helpItem xsi:nil="true" />
          <ns1:orderNumber>338459-20180530-00001804</ns1:orderNumber>
        </ns1:UiCancelRequestModel>
      </ns2:uiCancelRequestModels>
    </ns2:authoriCancel>
  </SOAP-ENV:Body>
...
*/
function authoriCancel($requestId, $uiCancelRequestModels) {
  ...
  $params = array('userAuthModel' => _convertClassObjectToArray($userAuthModel),
                  'intVal' => $requestId,
                  'uiCancelRequestModels' => $uiCancelRequestModels);
  ...
  try{
    //SOAP通信実行
    $result = $client->authoriCancel($params);
...
<html>
  <head>
    <title>authoriCancel | PaymentAPI</title>
    <meta charset="UTF-8">
    <style>
      pre,code {
        width:100%;
