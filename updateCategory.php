...
require_once('class/categoryUpdateRequest.php');
...
// 楽天へRMS APIを使って登録
list($reqXml, $httpStatusCode, $response) = updateCategory($categoryUpdateRequest);
...
/*
* shop.category.update APIのリクエストを行う
* xmlを作って curlでpostしてる
* @param 更新したいカテゴリー情報のクラスオブジェクト
* @return リクエストしたxml文字列, httpステータスコード, レスポンス文字列(xmlで返ってくる)
*/
function updateCategory($categoryUpdateRequest) {
  ...
  $url = RMS_API_CATEGORY_UPDATE;
  $ch = curl_init($url);
  
  $reqXml = _createRequestXml($categoryUpdateRequest);
...
*/
function _createRequestXml($categoryUpdateRequest) {

  // リクエストXMLのガワを作る
  $rootXml = new SimpleXMLElement('<?xml version="1.0" encoding="UTF-8"?><request/>');
  $categoryUpdateRequestXml = $rootXml->addChild('categoryUpdateRequest');
  
  // 受け取ったオブジェクトをarrayに変換
  $array = _convertClassObjectToArray($categoryUpdateRequest);
  
  _arrayToXml($array, $categoryUpdateRequestXml);  // リクエストのXMLをarray情報から作成する
...
<html>
  <head>
    <title>shop.category.update | CategoryAPI</title>
    <meta charset="UTF-8">
    <style>
      pre,code {
        width:100%;
