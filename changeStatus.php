 ...
 * */
$changeStatusRequestModel = new ChangeStatusRequestModel();
$changeStatusRequestModel->requestId = $requestId;

// 更新したい注文のステータス情報を設定
$orderStatusModel = new OrderStatusModel();
$orderStatusModel->orderNumber = "338459-20180531-00000720";
$orderStatusModel->statusName = "発送前入金待ち";

// 更新したい注文情報をリクエストに設定
$changeStatusRequestModel->orderStatusModel[] = $orderStatusModel;


// 楽天へRMS APIを使って送信
list($request, $httpStatusCode, $response) = changeStatus($changeStatusRequestModel);

//////////////// 関数群 ////////////////////

/***
 * 受注ステータスの変更 changeStatus　[非同期]
 * $requestIdはシステム内に保存しておくこと。
 ...
 * */
function changeStatus($changeStatusRequestModel) {
  ...
  $params = array('arg0' => _convertClassObjectToArray($userAuthModel),
    'arg1' => _convertClassObjectToArray($changeStatusRequestModel));
  ...
  try{
    //SOAP通信実行
    $result = $client->changeStatus($params);
...
<html>
  <head>
    <title>changeStatus | OrderAPI</title>
    <meta charset="UTF-8">
    <style>
      pre,code {
        width:100%;
