...
// バリエーション商品 全パターンの個別設定(本APIでは一部のバリエーションの在庫数だけを更新することはできない)
// 今回のサンプルパターンは下記4パターン想定でそれぞれ在庫数1で設定
//  サイズ：S, M
//  カラー：赤, 白
$horizontals = array('S', 'M');
$verticals = array('red', '白');
foreach($horizontals as $horizontal) {
  foreach($verticals as $vertical) {
    ...
    $inventory->inventoryCount = rand(0, 4);
...
<html>
  <head>
    <title>item.update(Variation) | ItemAPI</title>
    <meta charset="UTF-8">
    <style>
      pre,code {
        width:100%;
